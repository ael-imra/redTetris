...
module.exports = class SocketSubscription {
    ...
    joinRoom(name) {
        try {
            ...
            if (room && this.player) {
                this.io.to(room.name).emit('player joined', { name: this.player.name, room: room.name })
                this.socket.join(room.name)
                this.socket.emit('room joined', room)
            }
        } catch (error) {
    ...
    kickRoom(username) {
        try {
            if (this.player && this.player.room && RoomSubscription.kick.call(this, username)) {
                const room = this.player.room
                this.io.to(room.name).emit('message', { name: this.player.name, message: 'kicked from room' })
    ...
    sendMessage(message) {
        try {
            const msg = RoomSubscription.message.call(this, message)
            if (msg)
                this.io.to(this.player.room.name).emit('message', msg)
        } catch (error) {
            this.handleError(error)
        }
    }
        ...
        try {
            if (this.player && this.player.room) {
                ...
                if (RoomSubscription.exit.call(this)) {
                    this.io.to(room.name).emit('message', { name, message: 'exited from room' })
