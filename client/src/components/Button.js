...
const Button = ({ type, text, size, className, icon, onclick, sizeIcon, style = {}, disabled = false, font = 'lato' }) => {
	...
	} ${size === 'small' ? 'text_small__l' : size === 'large' ? (font === 'game' ? 'text__small__xxl' : 'text__small__xl') : 'text__small__xl'}  ${className || ''} btn__${size}`;
	const iconImg = () => {
