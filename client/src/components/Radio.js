...
const Radio = ({ name, option = [], style, className, onChange, defaultValue }) => {
  return (
    <div className={`radio flex flex__align-items__center flex__justify-content__center ${className}`} style={{ ...style }}>
      {option.map((item, key) => (
        <Option label={item.label} value={item.value} key={key} id={`radio_${key}`} name={name} icon={item?.icon && item.icon ? item.icon : null} onChange={onChange} defaultValue={defaultValue} />
      ))}
    </div>
  );
};
...
export const Option = ({ label, value, id, name, icon, onChange, defaultValue }) => {
  ...
  return (
    <div className='radio__box u__margin--small__l u__margin--small__r'>
      ...
      />
      <label className={`radio__box__checkbox flex flex__align-items__center flex__justify-content__center flex__direction__column ${refBox && refBox?.current && refBox.current.checked ? 'radio__box__checkbox__checked' : 'radio__box__checkbox__not-checked'}`} htmlFor={id}>
